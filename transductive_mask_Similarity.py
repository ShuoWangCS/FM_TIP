...
def training_loop(lowshot_dataset,novel_test_feats, num_classes, params, batchsize=1000, maxiters=1000, nTimes = 0):
    if os.path.exists('Model_SHOT1/' + params.name + '/' + params.dataset_name + '/' + params.backbone_name + '/LayerBestModel_' + str(params.alpha) + '_' + str(params.beta) + '/') == False:
        os.makedirs('Model_SHOT1/' + params.name + '/' + params.dataset_name + '/' + params.backbone_name + '/LayerBestModel_' + str(params.alpha) + '_' + str(params.beta) + '/')
    if os.path.exists('Model_SHOT1/' + params.name + '/' + params.dataset_name + '/' + params.backbone_name + '/' + str(nTimes) + '_' + str(params.alpha) + '_' + str(params.beta)) == False:
        os.makedirs('Model_SHOT1/' + params.name + '/' + params.dataset_name + '/' + params.backbone_name + '/' + str(nTimes) + '_' + str(params.alpha) + '_' + str(params.beta))
...
def eval_loop(data_loader, model, base_classes, novel_classes):
    ...
    top1 = None
    top5 = None
    no_novel_class = list(set(range(608)).difference(set(novel_classes)))
    all_labels = None
...
def parse_args():
    ...
    parser.add_argument('--name', default='transductive_mask_Similarity', type=str)
    parser.add_argument('--numclasses', default=608, type=int)
...
if __name__ == '__main__':
    ...
    for dataset_name in os.listdir('Features/'):
        for backbone_name in os.listdir('Features/' + dataset_name):
            ...
            test_feats = h5py.File('Features/' + dataset_name + '/' + backbone_name+ '/test.hdf5', 'r')
            ...
            params.backbone_name = backbone_name
            ...
            for alpha in [0.8]:
                ...
                end_ = max_epoach_fsl 
                params.alpha = alpha
                if os.path.exists('Model_SHOT1/' + params.name + '/' + params.dataset_name + '/' + params.backbone_name + '/LayerBestModel_' + str(params.alpha) + '_' + str(params.beta) + '/'):
                    len_results = len(os.listdir('Model_SHOT1/' + params.name + '/' + params.dataset_name + '/' + params.backbone_name + '/LayerBestModel_' + str(params.alpha) + '_' + str(params.beta) + '/'))
                ...
                for nTime in range(start_, end_):
                    ...
                    lowshot_dataset = LowShotDataset(novel_val_feats, novel_feats, base_classes, novel_classes, params)
